import { Link } from 'react-router-dom';
import { useSelector } from 'react-redux';
import { useDispatch } from 'react-redux';
import { useEffect, useState } from 'react'
import { getplaylistsByUser } from '../../store/playlist';
import { getAudio } from '../../store/playlistPlayer';
import './playlistsIndex.css'

const PlaylistYouMightLike = () => {
    const dispatch = useDispatch()
    const currentUser = useSelector(state => state.session.user)

    useEffect(() => {
        if(currentUser) dispatch(getplaylistsByUser(currentUser.id))
    }, [currentUser, dispatch])

    const playPlaylist = async (id) => {
      await dispatch(getAudio(id))
    }

  const playlistObject = useSelector(state => state.playlists.allplaylists)
  const playlists = Object.values(playlistObject);

  return (
    <div className='box'>
    <h2 className="playlistHeader">Take a look at the playlists from our Community</h2>
    <div className='playlistSectionAll'>
        {
          playlists?.map(playlist => (
            <div className='box'>
                <img className="playButton" alt="" src={"data:image/jpeg;base64,"} onClick={() => getAudio(playlist.id)}/>
                <img className="images" alt="" onClick={() => playPlaylist(playlist.id)} src={`${playlist.imageUrl}`}/>
                <div className="playlist">{playlist.title}</div>
            </div>
          ))
        }
    </div>
    </div>
  );
}

export default PlaylistYouMightLike;
